...
# ---------------------------------------------------------------------------
# 实例总限速（instance_quota_mbps）
# ---------------------------------------------------------------------------

def test_instance_quota_default_zero(tmp_path):
    """不写 = 不限（平铺模式），这是既有部署升级上来的默认形态。"""
    assert load_from(tmp_path, minimal()).instance_quota_mbps == 0.0


def test_instance_quota_parsed(tmp_path):
    cfg = load_from(tmp_path, minimal(extra="instance_quota_mbps: 800\n"))
    assert cfg.instance_quota_mbps == 800.0


def test_instance_quota_zero_explicit(tmp_path):
    """显式写 0 = 不限，与不写等价（写 API 取消总限速时会直接删字段，
    但手写 0 也必须合法）。"""
    cfg = load_from(tmp_path, minimal(extra="instance_quota_mbps: 0\n"))
    assert cfg.instance_quota_mbps == 0.0


def test_old_nic_quota_key_rejected_with_migration_hint(tmp_path):
    """旧字段名 nic_quota_mbps 显式拒绝并指出新名字与语义变化——静默
    忽略会让运维以为总限速还生效着。"""
    with pytest.raises(ValueError, match="instance_quota_mbps"):
        load_from(tmp_path, minimal(extra="nic_quota_mbps: 800\n"))


@pytest.mark.parametrize("text,match", [
    (minimal(extra="instance_quota_mbps: -1\n"),
     r"instance_quota_mbps 不能为负数"),
    (minimal(extra="instance_quota_mbps: 0.000004\n"), r"太小"),
    (minimal(extra="instance_quota_mbps: abc\n"),
     r"instance_quota_mbps 必须是数字"),
])
def test_instance_quota_rejections(tmp_path, text, match):
    with pytest.raises(ValueError, match=match):
        load_from(tmp_path, text)
