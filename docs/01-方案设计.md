...
| 数据可信 | 限额只有一份权威存储（本机 YAML 的 quotas/instance_quota_mbps；控制台页面与写 API 的修改也回写到它），实际用量可实时核对（持续超限告警） |
...
- **rl-limiter（Python，与 HAProxy 同机）**：把 cfg 解析出的清单与
  ...
  frontend，产出带宽视图（内置 Web 控制台；限额可在页面/写 API 上改，
  带令牌鉴权，回写 YAML 热生效）并做持续超限告警。
...
flowchart TD
    subgraph N1["一台 ECS（完整闭环）"]
        ...
        RL1["rl-limiter<br/>解析 cfg + 合并 quotas（5s 轮询热更新）<br/>每秒 show stat → 带宽视图 + 超限告警<br/>控制台 :8090（读免鉴权 / 改限额需令牌）"]
        CFG -->|"只读解析"| RL1
        YAML -->|"只读加载"| RL1
        CFG -->|"reload"| HA1
...
指定监听地址，**默认只绑 127.0.0.1**——读接口无鉴权，改限额需
`RL_API_TOKEN` 令牌；详见
