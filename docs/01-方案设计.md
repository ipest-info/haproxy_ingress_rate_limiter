...
**v3.0 落地补充**：上述数据模型在骨架里以 **MySQL** 落地为最小可用子集，并用 `docker compose` 一键部署。配置表：`envs`（环境配额 Mbps）、`env_targets`（环境↔(节点,frontend) 挂载点）、`haproxy_nodes`（受控节点接线）、`settings`（运行模式）；用量与心跳表：`usage_samples`（每秒样本）、`heartbeats`（各实例模式/配置版本）。**配置版本 = `envs`/`env_targets`/`settings` 三张配置表 `updated_at` 的最大值（秒）**，rl-limiter 每 5s 轮询版本，改库即热重载、秒级生效、无需重启（替代 §3.6 的 HTTP 长轮询）。建表脚本见 `deploy/mysql/schema.sql`，接入代码见 `rl_limiter/db.py`。
...
rl_limiter/       # Python 3.11 + asyncio 集中限速服务
  ...
  db.py           #   v3.0 MySQL 配置源（节点/配额/模式）与用量/心跳落库
  config.py       #   本地 YAML 配置（节点连接信息 + 引导配额，YAML 模式）
  loop.py         #   1s 主循环（配置优先于 tick、周期状态汇总）
tools/            # fake_haproxy.py（联调假节点）、mock_backend.py（后台桩）
deploy/           # mysql/schema.sql、docker/Dockerfile、systemd unit、haproxy 配置片段、tc 脚本
docker-compose.yml  # v3.0 一键部署：MySQL + 两台假 HAProxy + 限速服务
docs/
```
