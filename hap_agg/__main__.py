# hap_agg.__main__ —— hap-agg（HAProxy 监控聚合）服务入口。
#
...
try:
    ...
    SERVICE_VERSION = _metadata.version("hap-agg")
except Exception:  # pragma: no cover - 未安装场景
    SERVICE_VERSION = "dev"

ENV_API_TOKEN = "HAP_AGG_TOKEN"


async def _amain(cfg: configmod.AggConfig, log: logging.Logger,
                 yaml_path: str, port: int, bind: str,
                 api_token: str) -> None:
    aggregator = samplermod.Aggregator(cfg.targets, log)
    hub = webmod.AggHub(SERVICE_VERSION)
    ...
    tasks = [
        ...
        asyncio.create_task(
            webmod.run_web(port, hub, aggregator, log, bind=bind,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        epilog="目标清单在 -c 指定的 YAML 里（targets: 列表，见 "
               "deploy/hap-agg.example.yaml），也可以在页面/API 上"
               "批量导入（每行一条 'IP:port' 或 '名字 IP:port'，需设 "
               "HAP_AGG_TOKEN），导入结果回写 YAML。读接口无鉴权，默认只"
    ...
    try:
        ...
        print(f"hap-agg: 找不到配置文件 {args.config}（{e.strerror}）。\n"
              ...
              f"（示例见 deploy/hap-agg.example.yaml）。",
              file=sys.stderr)
        raise SystemExit(1)
