# hap_agg.config —— YAML 配置（目标清单 + 少量运行参数）：
#
#   log_level: info
...
# 手写注释会丢，头部注明）。运行期不轮询本文件——目标集的运行态以
# 进程内存为准（写 API 是唯一的运行期入口），手工编辑文件要重启才
# 生效；双写入口才需要合并逻辑，这里刻意不做。
...
from .sampler import Target, TargetError, parse_target_line
...
_HEADER = (
    ...
    "# 数据往返的限制）。字段说明见 deploy/hap-agg.example.yaml。\n"
)
