# hap_agg.sampler —— 多台 HAProxy 的监控聚合采样器（核心）。
#
# hap-agg 是**纯只读的跨机观测**：经各台机器 haproxy.cfg 里暴露的内网
# TCP stats socket（`stats socket ipv4@*:9999 level user` 之类，level
# admin 的也能用——本程序只读）批量采样，把 N 台 HAProxy 合并成一个
# 视图。它不写任何 HAProxy 状态，也不要求目标机器装任何东西。
#
# ## 采样与差分
#
