# hap_agg.haproxy —— HAProxy runtime API（stats socket）异步客户端。
#
# hap-agg 与各台 HAProxy 之间唯一的交互通道，且是**只读**的：每拍经
# show_stat / show_info 拉取各 frontend 的计数器与进程级指标。支持两种
# 接线：内网 TCP stats socket（聚合监控的主形态，haproxy.cfg 里
# `stats socket ipv4@<内网IP>:9999 level user`）与本机 unix socket
# （测试/同机场景）。两者只有"怎么建连"一步不同。
#
