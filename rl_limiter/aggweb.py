# rl_limiter.aggweb —— hap-agg 的 Web 视图与 API。
#
# 结构照搬 webconsole 的成熟做法（同样的安全边界、SSE 停机哨兵、令牌
# 鉴权），但面向的是**多目标聚合**：
#
#   - 读接口无鉴权，默认只绑回环，放内网靠防火墙；
#   - 写接口（批量导入/删除目标）必须带令牌（RL_API_TOKEN，与
#     rl-limiter 同一个约定：Authorization: Bearer 或 X-API-Token，
#     未配置则写接口整体 403）；
...
class AggHub:
    ...
    def overview(self, aggregator: aggmod.Aggregator) -> dict[str, Any]:
        return {
...
def render_prometheus(hub: AggHub, aggregator: aggmod.Aggregator) -> str:
...
def build_app(
    ...
    写接口（批量导入/删除目标）只在 yaml_path 非空时挂载，鉴权约定与
    rl-limiter 的写 API 完全一致（RL_API_TOKEN；未配置则 403）。
    """
    index_html = (_STATIC_DIR / "agg.html").read_bytes()
    ...
    async def handle_import_targets(request: web.Request) -> web.Response:
        ...
        async with write_lock:
            try:
                new = aggmod.parse_targets_text(text)
                added = aggregator.add_targets(new)
                await asyncio.to_thread(
                    aggconfig.save_targets, yaml_path, aggregator.targets())
            except (aggmod.TargetError, ValueError) as e:
    ...
    async def handle_delete_target(request: web.Request) -> web.Response:
        ...
        async with write_lock:
            ...
            if removed:
                try:
                    await asyncio.to_thread(
                        aggconfig.save_targets, yaml_path,
                        aggregator.targets())
