...
- **整机限速**（`host`，新装机器推荐）：一个速率类罩住本机网卡的**全部
  出向流量**，不按端口分类。队列树固定两个类，加多少个入口都不变，也没有
  按源端口分类的那一串坑。适合"一台机器 = 一份带宽"的场景。
- **按 frontend 限速**（`frontend`，默认）：按源端口把每个 frontend 的出向
  流量分到自己的速率类里。适合一台机器上多个入口各有各的限额。
...
**限速范围在控制台的「实例」页上切**（整机限速需要填一个正的整机限额，
不填直接拒——这个范围下没有各端口的限额可退，缺了就等于没限）。切范围会
重建本机网卡的队列树；只改整机限额则就地生效，不打断任何连接。
